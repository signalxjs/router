# SignalX router — shared agent guide
...
SignalX `router` (`signalxjs/router`) — Type-safe router for SignalX with SSR support and hooks. A pnpm monorepo (ESM, `"type": "module"`) with the package under `packages/`. Tech stack: TypeScript (strict), Vitest, oxlint; published to npm under the `@sigx` scope.
...
Never commit straight to `main`.** Repo: `signalxjs/router`, base branch `main`.
...
4. **Open a PR with Copilot as the reviewer.** Reference the issue so it auto-closes
   ...
   ```sh
   gh api --method POST repos/signalxjs/router/pulls/<pr>/requested_reviewers \
...
- `packages/router` → `@sigx/router` — Router for SignalX with SSR support and hooks.
...
- **Before merging a PR with user-facing changes, file an issue on the docs
  ...
  gh issue create --repo signalxjs/signalxjs.github.io \
    --title "router: <what changed>" \
    --body "Source: signalxjs/router#<pr>. <What needs documenting, and where on the site.> Not yet released."
  ```
...
- **When you cut a release** (push a `vX.Y.Z` tag), comment the release tag on
  ...
  gh issue comment <n> --repo signalxjs/signalxjs.github.io \
    --body "Released in router vX.Y.Z."
  ```
...
4. Adapt the repo-specific sections of `AGENTS.md`: the intro (what the repo is),
   "Build, Test, Lint", and "Packages". Replace every `router` with the repo name.
5. Keep the workflow, worktree, and conventions sections as-is — they are the
   shared standard.
6. Lock down `main`: `node scripts/apply-branch-protection.mjs signalxjs/router`.
