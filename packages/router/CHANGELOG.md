...
- **Aligned with SignalX core 0.12.** The core peer ranges (`@sigx/reactivity`, `@sigx/runtime-core`, `@sigx/runtime-dom`, `sigx`) move from `>=0.10.0 <0.11.0` to `^0.12.0` (== `>=0.12.0 <0.13.0`), and the router is rebuilt and validated against core `0.12.0`. Read the peer range, not the router version, to know which core a given router supports. ([#60](https://github.com/signalxjs/router/issues/60))
  - **No public API change.** The 0.10→0.12 window is additive for the router's core surface (`signal`, `component`, `defineInjectable`, `defineProvide`, `getCurrentInstance`, `runWithContext`, and the `App` / `Define` / `ComponentFactory` / `Plugin` types via the `sigx` barrel). Core 0.11's namespace-agnostic renderer rework touches only hosts that create elements directly — the router creates none — and 0.12 is a purely additive release (the new `@sigx/server` server-function pack). Slot accessors were already migrated to the presence-reflecting `slots.default?.()` form in 0.7.
  - **Core version is now a pnpm catalog.** The single source of truth is the `catalog:` block in `pnpm-workspace.yaml`; `package.json` references it with `"catalog:"` in both `peerDependencies` and `devDependencies`. `pnpm pack`/`publish` rewrites `"catalog:"` to the concrete `^0.12.0` range in the published manifest, so consumers see a normal semver range. Future core bumps are a one-line change to the catalog. The single-minor range is deliberate: `@sigx/reactivity` keeps reactive state in module-local variables, so a wider range that admitted two physical core copies would break reactivity — pnpm hoists exactly one 0.12.x copy.
- Navigation guards pass core's `asyncAdvice` option through `runWithContext`, so core's dev-only async-callback warning carries router-specific advice for async guards (resolve injectables at the top of the guard, before the first `await`) instead of naming an API guard authors never call. Wired as a no-op against core 0.10; **now live** with the move to core 0.12, which carries `RunWithContextOptions` (added in core 0.11 by signalxjs/core#292, for signalxjs/core#276). ([#56](https://github.com/signalxjs/router/issues/56))
