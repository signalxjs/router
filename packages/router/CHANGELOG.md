...
- Navigation guards pass core's `asyncAdvice` option through `runWithContext`, so core's dev-only async-callback warning carries router-specific advice for async guards (resolve injectables at the top of the guard, before the first `await`) instead of naming an API guard authors never call. Wired as a no-op against core 0.10; **now live** with the move to core 0.12, which carries `RunWithContextOptions` (added in core 0.11, signalxjs/core#276). ([#56](https://github.com/signalxjs/router/issues/56))
